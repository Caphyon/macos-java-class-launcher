...
/*
 *  main.m
 *  MacOS Java Class Launcher
 *
 *  Extended by Mihai Udrea on 11/14/13 for Cahpyon / Advanced Installer needs:
...
int launch(char *commandName) {
    ...
    NSMutableString *classPath = [[NSMutableString alloc] init];

    
    // Get the class path arguments
    NSArray *cpList = [infoDictionary objectForKey:@JVM_CLASS_PATH_KEY];
    if (cpList == nil)
    {
        // Get the class path from 
        [classPath appendFormat:@"-Djava.class.path=%@/Classes", javaPath];
    ...
    {
        // Get the class path from plist
        [classPath setString:@"-Djava.class.path="];
        for (NSString *option in cpList) {
            option = [option stringByReplacingOccurrencesOfString:@APP_ROOT_PREFIX withString:[mainBundle bundlePath]];
            [classPath appendFormat:@":%@", option];
        }
    }
    ...
    for (; j < argc; ++j) {
        ...
        NSAlert *alert = [[NSAlert alloc] init];
        [alert setAlertStyle:NSCriticalAlertStyle];
        [alert setMessageText:[NSString stringWithFormat:@"Argument %d = %s", j, argv[j]]];
        [alert runModal];
    }
    */
